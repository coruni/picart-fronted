...
export const removeSecure = (key: string) => secureStorage.remove(key);

export const clearSecure = () => secureStorage.clear();

// 用户数据相关（使用加密存储）
export const setUserData = (userData: Record<string, unknown>) =>
  encryptedStorage.set('user_data', userData, 60 * 24); // 24小时过期

export const getUserData = () => encryptedStorage.get('user_data');

export const removeUserData = () => encryptedStorage.remove('user_data');

// 应用设置相关（使用普通存储）
export const setAppSettings = (settings: Record<string, unknown>) =>
  secureStorage.set('app_settings', settings);

export const getAppSettings = () => secureStorage.get('app_settings');

// 临时数据相关（短期过期）
export const setTempData = (key: string, data: unknown, minutes = 30) =>
  secureStorage.set(`temp_${key}`, data, minutes);

export const getTempData = (key: string) => secureStorage.get(`temp_${key}`);
