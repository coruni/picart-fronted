...
export const Validator = {
  // 验证单个值
  validateValue(value: unknown, rules: ValidationRule[]): FieldValidationResult {
  ...
  // 验证对象
  validateObject(data: Record<string, unknown>, rules: ValidationRules): ValidationResult {
    const errors: string[] = [];
  ...
  // 应用单个规则
  applyRule(value: unknown, rule: ValidationRule): FieldValidationResult {
    // 必填验证
...
export const validate = {
  ...
  // 验证对象属性
  hasProperty: (obj: unknown, property: string) => {
...
export class FormValidator {
  ...
  // 设置数据
  setData(data: Record<string, unknown>): FormValidator {
    this.data = data;
    return this;
  }
